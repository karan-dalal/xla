...
absl::StatusOr<CudnnGraph> GetCudnnFlashAttentionOperationGraph(
  ...
  // Force bottom-right diagonal alignment for causal mask
  if (is_causal) {
    sdpa_options
  ...
  if (sliding_window_length > 0) {
    sdpa_options.set_diagonal_band_left_bound(sliding_window_length);
  }
...
absl::StatusOr<CudnnGraph> GetCudnnFlashAttentionBackwardOperationGraph(
  ...
  if (sliding_window_length > 0) {
    sdpa_backward_options.set_diagonal_band_left_bound(sliding_window_length);
  }
